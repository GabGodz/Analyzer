...
type Report = {
  ...
  events: KeyEvent[]
  stats: Stats
  meta: { started: number; last_seen: number } | null
}
...
function Histogram({ gaps }: { gaps: number[] }) {
  ...
  useEffect(() => {
    ...
    gaps.forEach(g => { bins[Math.min(Math.floor(g / bs), BINS - 1)]++ })
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ width: '100%', flex: 1, display: 'block', minHeight: 180 }}
    />
  )
}
...
export default function SessionPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!res.ok) { setError('Sessão não encontrada.'); return }
  ...
  if (error) return (
    <div style={{ padding: 40, color: 'var(--red)' }}>
      {error} <Link href="/dashboard">← Voltar</Link>
    </div>
  )
  if (!data) return (
  ...
  const vc = VERDICTS[stats.verdict] ?? VERDICTS.waiting
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ color: 'var(--muted)', fontSize: 11 }}>
              ...
              {data.meta?.started
                ? ` · ${new Date(data.meta.started).toLocaleString('pt-BR')}`
                : ''}
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--green)' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--green)', display: 'inline-block', animation: 'pulse 1.6s infinite' }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 12 }}>
        {[
          ...
          { label: 'Hold médio',  val: stats.avg_dur  !== null ? `${stats.avg_dur} ms`  : '—', color: valColor(stats.avg_dur,  25,  50) },
          { label: 'Gap médio',   val: stats.avg_gap  !== null ? `${stats.avg_gap} ms`  : '—', color: valColor(stats.avg_gap,  40, 100) },
          { label: 'Variância',   val: stats.variance !== null ? String(stats.variance)  : '—', color: valColor(stats.variance, 200, 1000) },
          { label: 'Desvio pad.', val: stats.stdev    !== null ? `${stats.stdev} ms`    : '—', color: valColor(stats.stdev,    15,  35) },
        ].map(m => (
          <div key={m.label} style={{ background: 'var(--panel)', border: '0.5px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}>
            <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.05em' }}>{m.label}</div>
      ...
      {/* verdict */}
      <div style={{ background: 'var(--panel)', border: '0.5px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 12, display: 'flex', gap: 14, alignItems: 'center' }}>
        <span style={{ fontSize: 22, flexShrink: 0 }}>{vc.icon}</span>
        <div>
          <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 4 }}>Diagnóstico</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 10 }}>
        ...
        <div style={{ background: 'var(--panel)', border: '0.5px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={boxTitle}>Log de eventos</div>
          ...
          <div style={{ overflowY: 'auto', maxHeight: 420 }}>
            {events.map((ev, i) => {
              ...
              const bc = cls === 'macro' ? '#e84545' : cls === 'suspect' ? '#f5a623' : cls === 'human' ? '#3ecf8e' : 'var(--muted)'
              const bb = cls === 'macro' ? 'rgba(232,69,69,.15)' : cls === 'suspect' ? 'rgba(245,166,35,.12)' : cls === 'human' ? 'rgba(62,207,142,.1)' : 'transparent'
              return (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px 90px 90px 1fr', padding: '4px 14px', borderBottom: i < events.length - 1 ? '0.5px solid rgba(37,42,58,.5)' : 'none' }}>
                  <span style={{ color: ev.key === 'z' ? 'var(--blue)' : 'var(--purple)', fontWeight: 700 }}>{ev.key.toUpperCase()}</span>
                  <span style={{ color: 'var(--muted)' }}>{ev.dur !== null ? `${ev.dur} ms` : '—'}</span>
                  <span style={{ color: 'var(--muted)' }}>{ev.gap !== null ? `${ev.gap} ms` : 'primeiro'}</span>
                  <span><span style={{ display: 'inline-block', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 4, background: bb, color: bc }}>{label}</span></span>
                </div>
              )
            })}
          </div>
        ...
        {/* histogram */}
        <div style={{ background: 'var(--panel)', border: '0.5px solid var(--border)', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={boxTitle}>Distribuição dos gaps (ms)</div>
...
const boxTitle: React.CSSProperties = {
  ...
  padding: '12px 14px 8px', borderBottom: '0.5px solid var(--border)',
}
